...
function validate(path) {
  assert(path && typeof path === 'string', 'Node path must be a non-empty string.');
  ...
  assert(path[path.length - 1] !== '/', 'Node path must not end with / character.');

  assert(!/\/\//.test(path), 'Node path must not contain empty node name.');

  assert(!/\/\.(\.)?(\/|$)/.test(path), 'Node path must not contain relative path(s).');
